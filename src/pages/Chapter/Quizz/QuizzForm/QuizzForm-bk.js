...
const EndModal = (props) => (
  ...
  >
    ...
    <Modal.Body>
      <p>
        {" "}
        Total Gained Points : {props.points}{" "}
        <i>
          {" "}
          ( you found {props.found_answer} questions out of{" "}
      ...
      if (user) {
        ...
        reference.once("value", (user_informations) => {
          ...
          if (!user_informations.val()) {
            ...
            const reference = database.ref(
              `users/${userId}/Progression/${userContext.user_informations.level}/${course}/${chapter}/progression`
            );
            reference.once("value", (user_informations) => {
              //console.log(user_informations.val())
              const {
                ...
                finished,
                onReset,
              } = user_informations.val();

              userContext.update_user_progression(
                found_questions / question_length
              );
              //update current index
              userContext.update_user_current_question_index(
                current_question_index
              );
              //update user points
      ...
      <div className="quizz-form-container">
        ...
        >
          {choices.length > 0
            ...
            : null}
          <Form.Field className="quizz-submit-btn" control={Button}>
            Valider
          </Form.Field>
        </Form>
      </div>
