...
const Progress = () => {
  ...
  const handleSetQuizzQuestionsNumber = (number) =>
    setQuizzQuestionsNumber(number);
  ...
  useEffect(() => {
    ...
    userContext.user_current_question_index,
          ...
          <p
            className={`${
              userContext.user_checked_false_answer
                ? "text-red shake-horizontal"
                : ""
            } ${
              userContext.user_check_true_answer ? "text-green bounce-top" : ""
            }`}
          >
            {userContext.user_checked_false_answer && (
              <Fragment>
                {" "}
                <Icon name="thumbs down" /> Mauvaise réponse{" "}
              </Fragment>
            )}
            {userContext.user_check_true_answer && (
              <Fragment>
                {" "}
                <Icon name="thumbs up" /> Bonne réponse{" "}
              </Fragment>
            )}
          </p>
          <p className="questions-repo text-center">
            {" "}
            {currentIndex + 1} / {quizzQuestionsNumber}{" "}
          </p>
