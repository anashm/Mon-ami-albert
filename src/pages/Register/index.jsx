...
const Register = () => {
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            const userRef = usersRef.child(userCredential.user.uid);
            const user = await userRef.once("value");

            let finalPhoneNumber = `${indicator}${phoneNumber}`
            finalPhoneNumber = finalPhoneNumber.replace(/\s/g, "");
            if(!finalPhoneNumber.includes('+')){
            ...
            if(!user.val()){
                ...
                await userRef.set({...userInfos});
                userContext.get_user_informations({...userInfos});
            }
    ...
    return (
        ...
        <div className = 'page-content'>
            <Container>
                <Divider />
                <Divider />
                <Title text="CRÉE TON COMPTE" textcentered centerOverlined />
                <Divider />

                <Form onSubmit = { handleSubmit } className = 'register-form'>
                    <Form.Field>
                        <input onChange = { e => setAlias(e.target.value) } placeholder='Alias' />
                    </Form.Field>
                    ...
                    {!!signInError &&  <p style = {{ marginTop : 10 , color : 'crimson' }}>{signInError}</p>}

                    <Divider />
                    
                    <Button  loading = {signInLoading} className = 'submit-btn w-100'>S'inscrire</Button>
                </Form>
            </Container>
        </div>
